      ...
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-slate-200/20 bg-slate-950/95 backdrop-blur-xl px-5 pt-3 pb-6 space-y-4 text-white shadow-2xl">
          <div className="py-2 border-b border-white/10">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">City Selected</p>
          ...
          <div className="space-y-2">
            {navLinks.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `block px-3 py-2 rounded-lg text-sm font-semibold transition-colors ${isActive ? 'text-red-500 bg-white/5' : 'text-slate-200 hover:text-white'
                  }`
                }
              >
                {item.name}
